...
int selectKnob(int n) { 
 ...
 return (j/100)/8; // value is between 0 and 1023, midi control values are 0-127
}
...
/*  Debug stuff
    ...
    Serial.println(y);
    delay(500);*/ 
    if(lastRead[x] != y) { // only send if the knob has actually moved
      Serial.write(176); // 
